...
class JiraSearch(object):
    ...
    def create_graph_image(self, graph_data, image_file, node_shape):
    ...
    def print_graph(self, graph_data, node_shape):
    ...
    def filter_duplicates(self, lst):
        ...
        def append_unique(self, acc, item):
            return acc if acc[-1][1] == item[1] else acc.append(item) or acc
        # srt_enum = sorted(enumerate(lst), key=lambda (i, val): val)
        srt_enum = sorted(enumerate(lst), key=lambda val,i: val)
        return [item[1] for item in sorted(append_unique, srt_enum, [srt_enum[0]])]
