...
class HInstall(object):
    def install(self, tool, destination, opt=None):
        ...
        if not os.access(priv_dest, os.W_OK):
            W().write("You need sudo privileges to write in {}\n".format(priv_dest))
        ...
        if tool == "e3":
            ...
            if os.path.exists(dest + "/e3"):
                overwrite = input(
                    "E3 found in the destination directory. "
                    "Overwrite existing E3? [Y/n]: "
                ).lower()
                if not overwrite == "y":
                    return
            ...
            try:
                ...
                ret_code = subprocess.check_call(
                    "{}/e3.install {} opt".format(path, dest, opt), shell=True
                )
            ...
            if os.path.exists(dest + "/ics_plc_factory"):
                overwrite = input(
                    "PLCFactory found in the destination directory. "
                    "Update PLCFactory? [Y/n]: "
                ).lower()
                if overwrite == "y":
                    existing = True
                else:
            ...
            repo_url = (
                "https://bitbucket.org/europeanspallationsource/ics_plc_factory.git"
            )
            ...
            if os.path.exists(dest + "/beast-config"):
                overwrite = input(
                    "BEAST found in the destination directory. " "Update BEAST? [Y/n]: "
                ).lower()
                if overwrite == "y":
                    existing = True
                else:
            ...
            with open(path + "/" + file_name, "wb") as f:
                ...
                for data in response.iter_content(chunk_size=4096):
                    ...
                    W().write("\r{}%".format(progress), "task")
                    sys.stdout.flush()
            ...
            W().write(
                "Tip: If you want to be able to start CSS from "
